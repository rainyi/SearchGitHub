...
- 화면 전환 메서드 제공 (`showResults`, `showDetail`, `pop`)
- ViewModel에 주입되어 사용
...
[RecentSearchUseCase] 최근 검색어 저장
    ↓
[SearchViewModel] router.showResults(for: query) 호출
    ↓
[AppRouter] path.append(.resultList(query: query))
    ↓
[SearchView] NavigationStack → ResultListView 표시
```

### 4.2 페이지네이션 플로우

```
[ResultListView] 스크롤 (마지막에서 5번째 셀)
    ↓
[ResultListViewModel] hasMorePages 확인
    ↓
[SearchRepositoriesUseCase] execute(query, page: nextPage)
    ↓
[GitHubAPIClient] 다음 페이지 요청
    ↓
[ResultListViewModel] 기존 결과 + 새 결과 병합
    ↓
[ResultListView] 리스트 업데이트
```
...
[User] 셀 탭
    ↓
[ResultListViewModel] repository 선택
    ↓
[ResultListViewModel] router.showDetail(url: repo.htmlUrl) 호출
    ↓
[AppRouter] path.append(.repositoryDetail(url: url))
    ↓
[ResultListView] NavigationStack → RepositoryDetailView 표시
```
...
final class MockAppRouter: AppRouter {
    ...
    override func showResults(for query: String) {
        lastRoute = .resultList(query: query)
    }
}
...
| 3rd-party 라이브러리 | **사용하지 않음** | 과제 규모, 기본기 평가 |
| 이미지 캐싱 | **URLCache + AsyncImage** | Kingfisher 불필요 |
| 에러 처리 수준 | **6가지 에러 타입** | 과제 요구사항 충족 |
| 페이지네이션 트리거 | **마지막에서 5번째 셀** | 미리 호출 구현 |
...
- 이 문서와 Planning.md를 기반으로:
  ...
  - 이 과제 규모에서 불필요하게 복잡한 부분이 있으면 단순화 제안해 달라.
- 최종적으로:
