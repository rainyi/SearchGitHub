...
| 레이어 | 테스트 유형 | 목표 커버리지 | 주요 대상 |
|--------|------------|--------------|----------|
| **Domain** | 단위 테스트 | 100% | UseCase, Entity, Error |
| **Data** | 단위/통합 테스트 | 80%+ | Repository, APIClient, Store |
| **Presentation** | 단위 테스트 | 70%+ | ViewModel 상태 관리, 네비게이션 |
| **UI** | 통합 테스트 | 핵심 플로우 | 검색→결과→상세 화면 흐름 |

**핵심 원칙:**
- Domain 레이어는 비즈니스 로직이므로 100% 커버리지 목표
- 테스트는 Given-When-Then 패턴으로 작성
- 각 Phase 완료 시 커버리지 체크 후 다음 Phase 진행
