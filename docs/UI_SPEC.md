...
3. **콘텐츠 영역** (조걸별 표시)
   ...
   - 각 항목:
     - 검색어 텍스트만 표시 (좌측 정렬, 시간/날짜 정보 없음)
     - X 버튼 (우측): 회색 원 배경 + 검정색 X (크기 8pt)
   - 전체 삭제 버튼: 목록 하단 우측에 빨간색으로 표시
   - 전체 삭제와 경계선 간격: 최소화 (4pt)
...
#### 자동완성 (검색어 입력 시)
- 트리거: 검색어 1글자 이상 입력
- 데이터 소스: 최근 검색어 목록 (최대 10개 중 필터링)
- 필터링: 입력 텍스트가 검색어에 포함되면 표시 (대소문자 무시)
- 표시 개수: 최대 5개
- UI:
  - 검색바 아래에 리스트 형태로 표시
  - 각 항목: 돋보기 아이콘 + 검색어 (좌측) + 날짜/시간 (우측)
  - 날짜 표시 형식:
    - 24시간 이내: 상대적 시간 (예: "28분 전", "5시간 전")
    - 24시간 이후: 날짜 형식 (예: "3월 9일")
- 동작: 항목 탭 시 해당 검색어로 즉시 검색 실행
